...
def collect_all_results() -> dict:
    ...
    # Power results
    power_file = PR / "power_results" / "power_breakdown.json"
    if power_file.exists():
        data["power"] = utils.load_results(power_file.parent, "power_breakdown")

    # Ablation results
    ablation_file = PR / "ablation" / "ablation_results.json"
...
def _read_external_table(name: str, data: dict) -> str:
    """Read a LaTeX table generated by another script (power, ablation)."""
    path = TABLES_DIR / name
    if path.exists():
        return path.read_text()
    return f"% {name} not found — run 07_power_estimation.py or 08_ablation.py\n"
...
def write_individual_tables(data: dict):
    ...
    # Also include power and ablation data in the tables dict
    tables = {
        ...
        "power_budget": lambda d: _read_external_table("power_budget.tex", d),
...
def write_paper_inputs(data: dict):
    ...
    with open(path, "w") as f:
        ...
        for name in ["dataset_stats", "model_architecture", "quantization_comparison",
                      "resolution_benchmark", "platform_comparison", "memory_budget",
                      "power_budget", "ablation"]:
        ...
        for name in ["dataset_samples", "quantization_tradeoff", "latency_comparison",
                      ...
                      "power_comparison", "power_fp32_vs_int8", "power_breakdown",
                      "power_timeline", "ablation_width_impact", "ablation_tradeoff"]:
...
def main():
    ...
    # Try to copy power/ablation figures if they exist
    for src_dir, prefix in [(PR / "power_results", ""),
                            (PR / "ablation", "")]:
        if src_dir.exists():
            for f in src_dir.glob("*.pdf"):
                dst = FIGURES_DIR / f.name
                if not dst.exists():
                    import shutil
                    shutil.copy2(f, dst)
                    print(f"  Copied: {f.name} → figures/")

    print(f"\n{'='*60}")
    print("All paper artifacts generated!")
    print(f"  Tables ({len(list(TABLES_DIR.glob('*.tex')))} files): {TABLES_DIR}/")
    print(f"  Figures ({len(list(FIGURES_DIR.glob('*.pdf')))} files): {FIGURES_DIR}/")
